...
MyServer::MyServer(QWidget *widget, QObject *parent) :QTcpServer(parent)
{
    ...
    } else
    {
      ...
      QSqlQuery query(db);
      query.exec("CREATE TABLE IF NOT EXISTS messages(date DATE, time TIME, sender TEXT, message TEXT)");
}
...
void MyServer::incomingConnection(qintptr handle)
{
    ...
    if (_widget != 0)
   {
        connect(client, SIGNAL(addUserToGui(QString)), (const QObject*)_widget, SLOT(onAddUserToGui(QString)));
        connect(client, SIGNAL(removeUserFromGui(QString)), (const QObject*)_widget, SLOT(onRemoveUserFromGui(QString)));
        connect(client, SIGNAL(messageToGui(QString,QString,QStringList)), (const QObject*)_widget, SLOT(onMessageToGui(QString,QString,QStringList)));
...
void MyServer::databaseFunction2(QString message, QString from)
{
    time_t rawtime;
    struct tm* timeinfo;
    time(&rawtime);
    ...
    QSqlQuery query(db);
  query.prepare("INSERT INTO messages(date,time,sender,message) VALUES(date('now'),time('now'), ?, ?)");
...
void MyServer::onMessageFromGui(QString message, const QStringList &users)
{
 ...
 databaseFunction2(message, "server");
}
